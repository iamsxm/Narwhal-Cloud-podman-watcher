...
- 入站去重 IP 优先通过宿主机 conntrack 与端口映射还原真实公网来源，兼容 Incus proxy、`incus network forward`、宿主机 iptables/nftables DNAT 和 Podman 端口映射；`10.x` 等代理网关地址不会冒充公网来源，不可还原时明确回退到容器网络命名空间的 `/proc/net`。默认只有数量大于 `ALERT_INBOUND_UNIQUE_IPS=10` 才产生重点告警。每轮最多读取 5000 条 conntrack 记录，Incus 网络转发配置每 4 分钟最多读取一次，并通过一次最多 500 条的容器 `ss` 快照把目标端口归属到通信进程，默认最多上报 100 条活动连接；全程不抓包、不扫描文件，不持续占用 CPU。
